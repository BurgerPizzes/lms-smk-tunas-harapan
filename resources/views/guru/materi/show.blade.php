        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
            @if($materi->tipe === 'file' && $materi->file_path)
                <div class="text-center py-8">
                    ...
                    <p class="text-sm font-medium text-gray-900 dark:text-white mb-1">{{ pathinfo($materi->file_path, PATHINFO_BASENAME) }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">{{ $materi->file_path_size ?? '' }}</p>
                    <a href="{{ Storage::url($materi->file_path) }}" download class="inline-flex items-center px-5 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors">
