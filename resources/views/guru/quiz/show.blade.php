...
<div class="flex items-center justify-between mb-6">
    <div>
        <a href="{{ route('guru.kelas.show', $quiz->class_id ?? '') }}" class="inline-flex items-center text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 mb-2 transition-colors">
        ...
        <div class="flex flex-wrap items-center gap-3 mt-1 text-sm text-gray-500 dark:text-gray-400">
            ...
            <span>&middot;</span>
            <span>{{ $quiz->questions->count() ?? 0 }} soal</span>
            <span>&middot;</span>
...
<div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
    <div class="border-b border-gray-200 dark:border-gray-700 px-6">
        <nav class="flex space-x-6 -mb-px" id="quiz-tabs">
            <button onclick="switchQuizTab('soal')" id="qtab-soal" class="qtab-btn py-3 border-b-2 border-indigo-500 text-indigo-600 dark:text-indigo-400 font-medium text-sm">
                Daftar Soal ({{ $quiz->questions->count() ?? 0 }})
            </button>
    ...
    <div id="qcontent-soal" class="qcontent-tab">
        @forelse($quiz->questions ?? [] as $index => $soal)
        <div class="p-5 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
            <div class="flex items-start justify-between">
                ...
                <div class="flex items-center space-x-1 ml-4 flex-shrink-0">
                    <a href="{{ route('guru.quiz.add-question', $quiz->id) }}" class="p-1.5 text-gray-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition-colors" title="Edit Soal">
                        <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg>
                    </a>
                    <form method="POST" action="{{ route('guru.quiz.delete-question', [$quiz->id, $soal->id]) }}" class="inline" onsubmit="return confirm('Hapus soal ini?')">
                        @csrf @method('DELETE')
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse($quiz->attempts ?? [] as $index => $hasil)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        ...
                        <td class="px-5 py-3 text-center text-sm text-gray-600 dark:text-gray-400 hidden md:table-cell">
                            <span class="text-green-600 dark:text-green-400 font-medium">{{ $hasil->total_benar ?? 0 }}</span>
                            <span class="text-gray-400"> / </span>
                            <span class="text-red-600 dark:text-red-400 font-medium">{{ $hasil->total_salah ?? 0 }}</span>
                        </td>
                        <td class="px-5 py-3 text-xs text-gray-500 dark:text-gray-400 hidden md:table-cell">{{ $hasil->waktu_mulai?->format('d M Y, H:i') ?? '-' }}</td>
                        <td class="px-5 py-3 text-xs text-gray-500 dark:text-gray-400 hidden lg:table-cell">{{ $hasil->waktu_selesai?->format('d M Y, H:i') ?? '-' }}</td>
                        <td class="px-5 py-3 text-xs text-gray-500 dark:text-gray-400">{{ $hasil->getFormattedDuration() ?? '-' }}</td>
                    </tr>
                    @empty
                    <tr>
