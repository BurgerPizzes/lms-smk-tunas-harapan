...
<div class="space-y-6">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500">
        ...
        <a href="{{ route('siswa.kelas.show', $materi->kelas) }}" class="hover:text-blue-600">{{ $materi->kelas?->nama_kelas }}</a>
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        <a href="{{ route('siswa.materi.index', $materi->kelas) }}" class="hover:text-blue-600">Materi</a>
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        ...
        <div class="flex-1 min-w-0 space-y-6">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex flex-wrap items-center gap-3 mt-2 text-sm text-gray-500">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{{ $materi->mapel?->nama }}</span>
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                ...
                @if($materi->file_path)
                    <div class="mt-8 pt-6 border-t border-gray-100">
                        <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">File Materi</h3>
                        <a href="{{ route('files.download', $materi->file_path) }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors text-sm font-medium">
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                <h2 class="text-lg font-semibold text-gray-900 mb-6 flex items-center gap-2">
                    ...
                    Komentar ({{ $materi->comments->count() }})
                </h2>

                {{-- Comment Form --}}
                <form action="{{ route('siswa.diskusi.store') }}" method="POST" class="mb-6">
                    @csrf
                    <input type="hidden" name="commentable_type" value="App\Models\Materi">
                    <input type="hidden" name="commentable_id" value="{{ $materi->id }}">
                    <div class="flex gap-3">
                        ...
                        <div class="flex-1">
                            <textarea name="body" rows="2" placeholder="Tulis komentar..." required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none resize-none"></textarea>
                            ...
                            <div class="flex-1">
                                <div class="bg-gray-50 rounded-xl px-4 py-3">
                                    <div class="flex items-center gap-2 mb-1">
                                        ...
                                        @if($comment->user && $comment->user->hasRole('guru'))
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="font-medium text-gray-900">{{ $materi->mapel?->nama }}</span>
                    </div>
                    <div class="flex items-center justify-between">
