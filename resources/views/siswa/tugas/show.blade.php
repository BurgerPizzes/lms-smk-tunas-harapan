...
<div class="space-y-6">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500">
        ...
        <a href="{{ route('siswa.kelas.show', $tugas->kelas) }}" class="hover:text-blue-600">{{ $tugas->kelas?->nama_kelas }}</a>
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        <a href="{{ route('siswa.tugas.index', $tugas->kelas) }}" class="hover:text-blue-600">Tugas</a>
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        ...
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-wrap items-center gap-3 mt-2 text-sm text-gray-500">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{{ $tugas->mapel?->nama }}</span>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl {{ $is_expired && !$submission ? 'bg-red-100' : 'bg-gray-100' }} flex items-center justify-center flex-shrink-0">
                        <svg class="w-6 h-6 {{ $is_expired && !$submission ? 'text-red-600' : 'text-gray-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        ...
                        @if($tugas->deadline)
                            <p class="text-lg font-bold {{ $is_expired && !$submission ? 'text-red-600' : 'text-gray-900' }}">{{ $tugas->deadline->translatedFormat('l, d F Y, H:i') }}</p>
                            @php
                            ...
                            @endif
                        @else
                            <p class="text-lg font-bold text-gray-900">Tidak ada deadline</p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                ...
                @if($tugas->file_attachment)
                    <div class="mt-6 pt-4 border-t border-gray-100">
                        ...
                        <a href="{{ route('files.download', $tugas->file_attachment) }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors text-sm">
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm">
                ...
                <div class="p-6">
                    ...
                    @if(!$submission)
                        ...
                        @else
                            <form action="{{ route('siswa.tugas.submit', $tugas) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                @csrf
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Jawaban / Catatan</label>
                                    <textarea name="jawaban" rows="5" placeholder="Tulis jawaban atau catatan kamu di sini..." required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none resize-none"></textarea>
                                </div>
                                <div>
                                    ...
                                    <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-blue-300 transition-colors">
                                        ...
                                        <p class="text-xs text-gray-400 mt-1">PDF, DOC, PPT, XLS, JPG, PNG, ZIP (Maks. 100MB)</p>
                                    </div>
                                </div>
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center gap-3 bg-blue-50 rounded-xl p-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-blue-600">Dikumpulkan pada {{ $submission->submitted_at?->translatedFormat('d M Y, H:i') }}</p>
                                </div>
                            </div>
                            @if($submission->konten)
                                <div class="bg-gray-50 rounded-xl p-4">
                                    <p class="text-xs font-medium text-gray-500 mb-1">Jawaban:</p>
                                    <p class="text-sm text-gray-700 whitespace-pre-wrap">{{ $submission->konten }}</p>
                                </div>
                            @endif
                            @if($submission->file_path)
                                <a href="{{ route('files.download', $submission->file_path) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-lg hover:bg-gray-100 text-sm text-gray-700">
                    ...
                    @else
                        <div class="space-y-4">
                            ...
                            <div class="bg-gray-50 rounded-xl p-4 space-y-2">
                                <p class="text-xs font-medium text-gray-500">Dikumpulkan: {{ $submission->submitted_at?->translatedFormat('d M Y, H:i') }}</p>
                                @if($submission->konten)
                                    <p class="text-sm text-gray-700 whitespace-pre-wrap">{{ $submission->konten }}</p>
                                @endif
                                @if($submission->file_path)
                                    <a href="{{ route('files.download', $submission->file_path) }}" class="inline-flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-700">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                        Unduh File
                                    </a>
                                @endif
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                <h2 class="text-base font-semibold text-gray-900 mb-4 flex items-center gap-2">
                    ...
                    Komentar ({{ $tugas->comments->count() }})
                </h2>
                <form action="{{ route('siswa.diskusi.store') }}" method="POST" class="mb-6">
                    @csrf
                    <input type="hidden" name="commentable_type" value="App\Models\Tugas">
                    <input type="hidden" name="commentable_id" value="{{ $tugas->id }}">
                    <div class="flex gap-3">
                        ...
                        <div class="flex-1">
                            <textarea name="body" rows="2" placeholder="Tulis pertanya atau komentar..." required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none resize-none"></textarea>
                ...
                <div class="space-y-4 max-h-80 overflow-y-auto">
                    @forelse($tugas->comments as $comment)
                        <div class="flex gap-3">
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="font-medium text-gray-900">{{ $tugas->mapel?->nama }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500">Kelas</span>
                        <span class="font-medium text-gray-900">{{ $tugas->kelas?->nama_kelas }}</span>
                    </div>
                    <div class="flex items-center justify-between">
